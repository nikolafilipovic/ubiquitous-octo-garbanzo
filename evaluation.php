    ...
    */
    ...
    if ($user_status != NULL){
         if ($user_status->exists()){
            $us = 1;
            // echo $user_status->user_email;
            $zap_email = $user_status->user_email;
            $zap_user_id = $user_status->ID;
         }else{
            $us = 0;
         }     
    }else{
     ...
     // echo $user;
      ...
      if (array_key_exists("analytics",$data)){
            ...
            if ($us == 1){
                $zap_data = [
                "email" => $user_status->user_email,
                "user_id" => $user_status->ID,
                "street_address" => str_replace('+', ' ', $properties['street_address']),
                "city" => str_replace('+', ' ', $properties['city']),
                "state" => str_replace('+', ' ', $properties['state']),
                "zip" => $zip,
                "property_area" => $sqft,
                "beds" => $beds,
                "baths" => $baths,
    ...
    <div class="faded">
        ...
        <div class="eval-header">
            ...
            <div class="eval-header-content">
                ...
                <h3 class="text-center"><?= $property['address']; ?></h3>
                <p><?= $property['city'] ?>, <?= $property['state'] ?> <?= $property['zip'] ?> at</p>
                <h1>$<?= number_format($analytics['avm_high'],0); ?></h1>
            </div>
            <?php else: ?>
...
<div class="eval-body">
    ...
    <div class="eval-floating-container">
        <div class="eval-floating-block np-top">
            ...
            <a href="/questionaire/<?php echo "?street_address=$properties[street_address]&state=$properties[state]&city=$properties[city]&zip=$zip"; ?>" class="btn btn-block">Compare Estimates <i class="far fa-arrow-right"></i></a>
            <a href="/selling-options/<?php echo "?street_address=$properties[street_address]&state=$properties[state]&city=$properties[city]&zip=$zip"; ?>" class="btn btn-block inverted">No, I'm ready to sell now <i class="far fa-arrow-right"></i></a>
        </div>
    </div>
            ...
            <div class="row row-eq-height">
                ...
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12 report-content">
                  <?php if($us == 1): ?>
                    ...
                    <a href="/evaluation-unlocked/<?= "?street_address=$properties[street_address]&state=$properties[state]&city=$properties[city]&zip=$zip&property_area=$sqft&beds=$beds&baths=$baths&estimate=$estimate"; ?>" class="btn btn-block" id="locked">I'm the owner. Show me! <i class="far fa-arrow-right"></i></a>
                    
                  <?php else: ?>
                    
                    <i class="far fa-lock-alt lock"></i>
                    <p>See property details, comparable home insights and neighborhood trends.</p>
                    <a  href="#" data-toggle="modal" data-target="#unlock-modal" class="btn btn-block" id="locked">I'm the owner. Show me! <i class="far fa-arrow-right"></i></a>
        ...
        jQuery( "#home_search_dropdown .items" ).on( "click", ".item", function(){
          ...
          //console.log(mode);
            ...
            let street_address = arr[3];            
              ...
              window.location="/evaluation/?country="+country+"&state="+state+"&city="+city+"&street_address="+street_address;
